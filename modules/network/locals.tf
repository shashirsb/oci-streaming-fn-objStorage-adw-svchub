...
db_ingress_seclist = [
  ...
  bastion_ingress_seclist = [
    {
      description = "Allow Bastion host to accept all incoming request. Required for when using OCI Bastion.",
      source      = local.anywhere,
      source_type = "CIDR_BLOCK",
      protocol    = local.all_protocols,
      port        = -1,
  ...
  operator_ingress_seclist = [
    {
      description = "Allow Operator host to accept all incoming request from Bastion. Required for when using VNC.",
      protocol    = local.tcp_protocol,
      port        = 5901,
      source      = local.bastion_subnet,
      source_type = "CIDR_BLOCK",
