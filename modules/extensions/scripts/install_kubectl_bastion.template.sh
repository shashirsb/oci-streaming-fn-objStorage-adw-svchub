...
if [ ! -f .kubectl_completed ]; then
  while [ ! -f /home/opc/bastion.finish ]; 
  ...
  if [ ${ol} = 8 ]; then
    ...
    echo "sudo yum install -y oracle-olcne-release-el7"
    sudo yum install -y oracle-olcne-release-el7 > /dev/null 2>&1

    echo "sudo yum-config-manager --enable ol7_olcne14"
    sudo yum-config-manager --enable ol7_olcne14 > /dev/null 2>&1

    echo "sudo yum install -y kubectl git"
