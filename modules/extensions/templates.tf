...
locals {
  ...
  generate_kubeconfig_bastion_template = templatefile("${path.module}/scripts/generate_kubeconfig_bastion.template.sh",
    {
      cluster-id = var.cluster_id
      region     = var.region
      ssh_private_key_path = var.ssh_private_key_path
      operator_ip = var.operator_private_ip
    }
  )
  ...
  install_vnc_template = templatefile("${path.module}/scripts/install_vnc.template.sh", {})

  install_vncstart_template = templatefile("${path.module}/scripts/install_vncstart.template.sh", {})
 ...
 install_kubectl_bastion_template = templatefile("${path.module}/scripts/install_kubectl_bastion.template.sh",
    {
      ol = var.operator_os_version
    }
  )

 install_xauth_template = templatefile("${path.module}/scripts/install_xauth.template.sh", {})

 install_wktui_template = templatefile("${path.module}/scripts/install_wktui.template.sh", {})

 install_wls_template = templatefile("${path.module}/scripts/install_wls.template.sh", {})
